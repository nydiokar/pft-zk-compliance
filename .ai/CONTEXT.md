...
| `ZK-15e` Implement non-native verifier equation | [ ] | The verifier-facing limb ABI is now fixed: `P.x`, `P.y`, `R.x`, `R.y`, `s`, and `e` are represented as four 64-bit little-endian limbs, derived from canonical inputs during sidecar normalization and validated by shared helpers. Remaining work is replacing the staged scalar relation with the real non-native Schnorr-over-Pasta verification equation in the circuit. |
...
- **[ZK-15e limb ABI partial]** Shared helpers now decompose canonical Schnorr material into the future verifier witness shape: decoded affine Pallas `P` and `R` coordinates plus scalar `s` and challenge `e`, each as four 64-bit little-endian limbs. Reconstruction rejects truncated/overflowed limbs, non-canonical field/scalar values, and off-curve point coordinates. The sidecar derives these limbs during request normalization, but the circuit still needs the actual non-native arithmetic gates.
